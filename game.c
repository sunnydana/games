...
typedef struct { //typedef is for a shortcut name for a data type to make code easier to read. (keeps you from having to use struct everytime)
    int x;
    int y;
    int w;
    int h;
...
int main(){
    ...
    SDL_Init(SDL_INIT_VIDEO); //initializes the SDL library so you can use it to create a window, draw graphics, or do other video-related things.
    SDL_Window* window = SDL_CreateWindow("Dodge Game", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 650, 490, 0); //Creates a window on the screen where your game(or graphics) will appear.
    ...
    while (running) {
        ...
        SDL_SetRenderDrawColor(renderer, 0, 255, 0, 0); // Set the drawing color to black (with full transparency), so future things (like the background or shapes) will be drawn using this color.
        SDL_RenderClear(renderer); // Clear the screen (or window) and fill it with the current draw color.

        SDL_Rect rect = { player.x, player.y, player.w, player.h }; // Create an SDL rectangle called rect that has the same position and size as the player.
        SDL_SetRenderDrawColor(renderer, 0 , 0 , 255, 0); // Set the drawing color to blue, with full transparency
        SDL_RenderFillRect(renderer, &rect); // Draw a filled rectangle on the screen using the current draw color, in the position and size of rect.
    ...
    SDL_Quit(); //Shut down SDL completely and clean up all the systems it initialized(like video,audio,input,etc).
...
//This line tells SDL: "Get ready! I want to work with graphics (like creating a window or displaying things.)"

//16
// SDL_Window* window -> sets up a varibale named window that will point to (hold the address of) the window you create.
// window = a box that stores the window you just made, so you can use it later(for drawing, closing, etc)
...
//"Dodge Game" -> title of the window(You'll see this text at the top of the window)
//SDL_WINDOWPOS_CENTERED -> X position
//SDL_WINDOWPOS_CENTERED -> Y position
// 650 -> Width of the window
// 490 -> Height of the window
// 0 -> Special flags(0 means no extra options)
//   Creates a window titled 'Dodge Game', centers it on the screen, makes it 650×490 pixels,
//    and gives you a way to refer to it using the window variable.

//17
//SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, 0);
//🧱 What’s a renderer?
//Think of your window as a blank canvas 🎨, and the renderer is your paintbrush 🖌️. 
//You must have a renderer if you want to draw anything on the screen (like your game graphics).

//SDL_Renderer* renderer         // This creates a variable to store your renderer.
//SDL_CreateRenderer(...)        // This function actually creates the renderer.

//(window,                      // Which window to draw on? (use the one we created earlier)
// -1,                          // Let SDL choose the best graphics driver
//  0)                          // No special settings (basic renderer)

// Line 17 says:
//"Give me a drawing tool (renderer) for this window so I can start putting things on the screen!"
//And it stores that tool in the variable renderer, which you’ll use later to do things like:
//Clear the screen
//Draw rectangles, images, text
//Update the screen


// 25
// bool running = true;
// It's often used in things like game loops or programs that run continuously until the user quits:
// while (running) {
//  game logic
// }
...
// 26
// SDL_Event event;
// SDL_Event is a special structure that holds information about things that happen, like
// Pressing a key
// Clicking a mouse
// Closing a window
// Moving the mouse
// Etc
// You use SDL_Event to listen for and handle user input.
...
// SDL_QUIT is a special event that SDL sends when the user clicks the close button on the window
// running = false; sets your game loop to false, so the loop ends.
// 🧠 So this is how you let the user gracefully exit your SDL program by closing the window.

//31
// SDL_KEYDOWN is an event type that SDL uses when any key is pressed on the keyboard(but not released yet).
//It works inside the SDL event loop

//32 
// event.key.keysym.sym   tells you which key was pressed.
// SDLK_LEFT    means the left arrow key.
// player.x -= 30;   decreases the player's X position by 30, which moves them left on the screen.

//33
// event.key.keysym.sym checks which key was pressed
// SDLK_RIGHT is the right arrow key
// player.x += 30; increases the player's X position by 30, so the player moves right on the screen.

//35
// event.key.keysym.sym tells which key was pressed.
// SDLK_UP is the up arrow
// player.y -= 30; decreases the player's y position, which moves the player up on the screen
//  (because in SDL, the top-left is (0,0), and Y increases as you go down).

//36
// event.key.keysym.sym checks which key was pressed.
// SDLK_DOWN means the down arrow.
// player.y += 30; increases the player's vertical position, so the player moves down on the screen
...
// That sets the color that SDL_RendererClear will use to fill the entire screen.
...
// SDL_RenderPresent(renderer); shows that hidden screen on the actual window(front buffer)

//48
// 16 ms because 16 miliseconds = 1/60th of a second, and most games aim for 60 frames
//per second(FPS)
...
// 51
// When your program ends, SDL doesn't automatically free all the memory it used. 
...
//If  you don't call this, your program might leak memory or leave system resources open.
